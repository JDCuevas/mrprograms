...
public class TwitterKeyWorkDriver {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = new Job();
        job.setJarByClass(edu.uprm.cse.bigdata.mrtwo.TwitterKeyWorkDriver.class);
        job.setJobName("Count NotStopWords");

        FileInputFormat.addInputPath(job, new Path(args[0]));
        FileOutputFormat.setOutputPath(job, new Path(args[1]));

        job.setMapperClass(edu.uprm.cse.bigdata.mrtwo.TwitterKeyWordMapper.class);
        job.setReducerClass(edu.uprm.cse.bigdata.mrtwo.TwitterKeyWorkReducer.class);
